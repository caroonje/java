...
//        -순차적으로 비교값을 설정해야함
//        -가각의 case문 끝에 있는 break;를 생략하면 안된다
//        -break;를 생략시 break를 만날때 까지 아래에 있는 case문을 실행함
//        -모든 case문과 일치않을 경우 default문이 실행됨
//        -default문은 생략이 가능함(모든 조건을 case문에 입력 시,사용자가 의도적으로 실행하지 않을 경우)
//        사용법:
//        swich (조건값){
//        case 조건값 1:
//        비교값이 조건값과 동일할 경우 실행할 소스코드...
//        break;
//        case 조검값2 :
//        비교값이 조건값2와 동일할 경우 실행할 소스 코드...
//        break;
//        case 조검값n :
//        비교값이 조건값n와 동일할 경우 실행할 소스 코드...
//        break;
//        default:
//        비교값이 모든 조건값과 일치하지 않을 경우 실행할 소스 코드...
//        break;
//    }

//        Math : 자바에서 제공하는 수학관련 클래스
//        Math.random() : 0.0 ~ 0.9까지의 임의의 숫자를 촐력
...
//        if문은 범용적으로 사용하기에 좋음
//        switch ~ case 문은 조건값을 기준으로 case 문의 값과 동일한지 판단함
//        switch case문은 고정된 값을 비교 시 사용하기에 좋음
        System.out.println("\n----- else if 문으로 대체하기 -----\n");
        if (day ==1){
...
//      고정된 값을 이용하여 만들때는 switch~case문
//      범위값을 이용하여 만들때는 if~ elseif문을 사용
